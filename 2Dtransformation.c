Write an interactive program for following basic transformations.
1) Translation
2) Rotation (with respect to arbitrary point also)
3) Scaling (with respect to arbitrary point also)
4) Reflection about x-axis, y-axis, y=x, y=-x and origin
5) Reflection about a line Y=mX+c 
6) Shearing about x-axis and y-axis
  ...
  while(count==-1){
    ...
    switch(ans)
    {
          ...
          if(choice2==1)
	  {
          	...
          	scanf("%f", &theta);
          	theta *= (3.141/180);//converting degree into radians
          ...
          if(choice2==1)
	  {
		...
		float matrix[][3] = {{-1,0,0},{0,-1,0},{0,0,1}};
          ...
          {
 		float m,c;
		printf("\nEnter the value of slove of line");
		scanf("%f",&m);
